...
#%%
#Define Functions - Helpful for styling the dataframe 
...
#%%
cols_agg_analysis = ['Comments added', 'Shares', 'Dislikes', 'Likes', 'Subscribers lost',
...
#12 months prior to the most recent video
metric_date_12mo = df_agg_diff['Video publish time'].max() - pd.DateOffset(months=12)

df_agg_last_12mo = df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo]

median_agg = df_agg_last_12mo[cols_agg_analysis].median()
#%%
...
if add_sidebar == 'Aggregate Metrics':
    ...
    metric_medians6mo = metric_medians6mo.select_dtypes(include=np.number).median()

    metric_date_12mo = df_agg_metrics['Video publish time'].max() - pd.DateOffset(months=12)
    metric_medians12mo = df_agg_metrics[df_agg_metrics['Video publish time'] >= metric_date_12mo]
    metric_medians12mo = metric_medians12mo.select_dtypes(include=np.number).median()

    col1, col2, col3, col4, col5 = st.columns(5)
    columns = [col1, col2, col3, col4, col5]

    count = 0
    for i in metric_medians6mo.index:  #loops thru all the labels
        with columns[count]:
    ...
    col_subset = ['Video title','Publish_date','Views','Likes','Subscribers','Shares','Comments added','RPM(USD)','Average % viewed',
                             'Avg_duration_sec', 'Engagement_ratio','Views / sub gained']
    df_agg_diff_final = df_agg_diff.loc[:,col_subset].reset_index(drop=True)

    #present the dataframe with some nice red and green colors
    #also convert to a percentage the numeric cols
    st.dataframe(df_agg_diff_final.style.
                 applymap(style_negative, props='color:red').
                 applymap(style_positive, props='color:green').
                 format(format_percentage, subset=col_subset[2:]))
...
if add_sidebar == 'Individual Video Analysis':
    ...
    first_30d = agg_time_filtered[agg_time_filtered['days_published'].between(0,30)].sort_values(by='days_published')
    ...
    fig2.add_trace(go.Scatter(
            x = first_30d['days_published'],
            y = first_30d['Views'].cumsum(),
            mode = 'lines',
            name = f'Current video:\n{video_select[:20]}',
